...
class SourceBundle(DataSource):
    ...
    ``SourceBundle.aread`` is called with.
    """
    ...
    async def aread(self, **kwargs: Any) -> dict[str, Any]:
        """Read all sources concurrently and return a named dict of results.

        Args:
            **kwargs: Forwarded verbatim to every source's ``aread()`` call.

        Returns:
            ``{source_name: result}`` for each source in the bundle.
        """
        names = list(self.sources.keys())
        results = await asyncio.gather(*[self.sources[name].aread(**kwargs) for name in names])
